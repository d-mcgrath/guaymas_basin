...
checkm2_bins = read_tsv('flat_files/andreas_mags_checkm2_quality_report.tsv', show_col_types = FALSE) |>
  rename_with(~ str_to_lower(.x)) |>
  select(-`translation_table used`)


gtdb_bins = read_tsv('flat_files/andreas_gtdbtk_archaea.tsv', show_col_types = FALSE) |>
...
magSummary = dasTool_summary_dec_2022 |>
  left_join(checkm2_bins |> rename(bin = name), by = 'bin') |>
  left_join(gtdb_bins |> rename(bin = mag_name), by = 'bin') |>
...
magSummary = magSummary |>
  ...
  mutate(mag_name = paste0(
    'Guaymas_MAG_P_',
    1:nrow(magSummary) |> str_pad(width = 3, side = 'left', pad = '0')),
    .before = 1)
...
# Figure 2
magSummary = read_tsv('~/Documents/guaymas_analysis/flat_files/filteredMagSummary.tsv')

envGeoData = read_tsv('~/Documents/guaymas_analysis/flat_files/environmentalGeochemicalData.tsv')
...
guay_relAbun_plot_data = guay_relAbun_plot_data |>
  ...
  relocate(c(mag_name, envGeoData$sample))
...
# pdf(
#   file = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/figure_2.pdf',
#   width = 7,
#   height = 9)
# draw(guay_relAbun_complexHeatmap,
#      merge_legend = TRUE)
...
# pdf(
#   file = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/figure_3.pdf',
#   width = 7,
#   height = 9)
...
nmds_scores_FINAL = scores(nmds_result_FINAL, "site") |>
  ...
  mutate(sample = rownames(.),
         sample = paste0('U', sample)) |>
  as_tibble() |>
  left_join(envGeoData, by = 'sample') |>
...
ggsave(
  filename = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/figure_4.pdf',
...
abund = guay_relAbun_files |>
  ...
  left_join(magSummary |> select(mag_name, bin), by = 'bin') |>
  relocate(mag_name, .before = 1) |>
...
sample_groups_depth = envGeoData |>
...
sample_groups_temperature = envGeoData |>
  rename(depth = 2, temperature = 3) |>
  select(sample, depth, temperature) |>
  mutate(depth_group = case_when(
    depth <= 15 ~ 'shallow',
...
temperature_mag_size_regression_plot =
  ...
  annotate('text', x = 45, y = 2.4,
           label = 'italic(p) == 5.02~x~10^{-8}',
           size = 4.2, parse = TRUE) +
  annotate('text', x = 42.7, y = 2.25,
           label = TeX('$R^{2}_{adj} = .72'),
           size = 4.2, parse = TRUE) +
  theme_bw() +
  theme(
...
#   filename = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/figure_5.pdf',
#   width = 10,
#   height = 10,
#   dpi = 500,
#   units = 'in')
...
# ggsave(
#   plot = figure_6,
#   filename = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/figure_6.pdf',
#   width = 10,
#   height = 5,
#   dpi = 500,
#   units = 'in')
...
mag_dist_plot_archaea <- magSummary |>
  group_by(phylum) |>
  summarize(freq = n()) |>
  arrange(desc(freq)) |>
  mutate(x = row_number(),
...
pdf(
  file = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/supplementary_figure_3.pdf',
  width = 7,
  height = 9)
draw(guay_relAbun_complexHeatmap_order,
     merge_legend = TRUE)
...
# ggsave(
#   filename = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/supplementary_figure_4.pdf',
...
## Supplementary Figure 5. Taxonomic order-specific relative abundance of Chloroflexota and Thermoproteota MAGs in metagenomic samples from drill sites U1545B and U1547B
...
# pdf(file = '~/Documents/guaymas_analysis/june_15_all_manuscript_docs/supplementary_figure_3.pdf',
#     width = 7,
#     height = 8)
# draw(guay_relAbun_complexHeatmap_thermo_chloro_1545_1547,
#      merge_legend = TRUE)
